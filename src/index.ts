#!/usr/bin/env node
/**
 * mx-fiscal-mcp-server entry point.
 *
...
import { MAX_XML_CHARS, SERVER_NAME, SERVER_VERSION } from "./constants.js";
...
function runHttp(): void {
  ...
  // A CFDI arrives as a JSON string: allow the XML cap plus room for escaping.
  const bodyLimit = Math.ceil(MAX_XML_CHARS * 1.25) + 64 * 1024;
  app.all("/mcp", guard, express.json({ limit: bodyLimit }), (req: Request, res: Response) => {
